// LINKS v237 — exact v235 Home behavior; player-page fixes run only away from Home. No data writes.
...
[class*="pool-category"],[class*="pool-category"] *:not(button):not(a){background-color:#000!important}
 ...
 if(isHome){
  ...
  const playerFixes=`<style id="links-v237-player-fixes">.tabs [data-tab="standings"],#specialTabs [data-special-tab="standings"]{opacity:1!important;filter:none!important;cursor:pointer!important;pointer-events:auto!important}.team .rec{font-weight:800;opacity:1;margin:2px 0 4px}</style><script id="links-v237-player-fixes-script">(()=>{let busy=false;const fix=()=>{if(busy)return;busy=true;try{document.querySelectorAll('.tabs [data-tab="standings"],#specialTabs [data-special-tab="standings"]').forEach(b=>{b.disabled=false;b.removeAttribute('disabled');b.style.removeProperty('opacity');b.style.removeProperty('filter');b.style.removeProperty('pointer-events')});document.querySelectorAll('.team').forEach(team=>{const logo=team.querySelector('.logo-pad'),rec=team.querySelector('.rec');if(logo&&rec&&logo.nextElementSibling!==rec)logo.insertAdjacentElement('afterend',rec)});const status=document.getElementById('compareStatus'),note=document.getElementById('compareNote'),body=document.getElementById('compareBody');if(status&&note&&body&&/Open/i.test(status.textContent||'')&&/Everyone's picks appear after/i.test(note.textContent||'')){note.textContent="🔒 Projected standings unlock after this week's picks lock so nobody can see another player's selections early.";body.innerHTML='<div class="card" style="text-align:center"><b>Projected standings are protected until picks lock.</b></div>'}}finally{busy=false}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{fix();new MutationObserver(fix).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','class'])},{once:true});else{fix();new MutationObserver(fix).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','class'])}}})();</script>`;
  html=html.replace("</body>",playerFixes+"\n</body>");
 }
 const headers=new Headers(response.headers);headers.delete("content-length");headers.set("cache-control","no-cache, no-store, must-revalidate");headers.set("x-links-build",isHome?"v235":"v237");return new Response(html,{status:response.status,statusText:response.statusText,headers});
}

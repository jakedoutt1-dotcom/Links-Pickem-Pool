...
const ReminderQueue={
 ...
 all(){try{return JSON.parse(localStorage.getItem(this.key)||"[]")}catch{return[]}},
 send(player){const a=this.all();a.unshift({player,at:new Date().toISOString(),type:"Missing picks · local draft only"});localStorage.setItem(this.key,JSON.stringify(a.slice(0,50)));AuditLog.add("REMINDER PREPARED",player+" · delivery service not connected");return a[0]}
};
function mountEntrantManager(){
 ...
 const send=name=>{ReminderQueue.send(name);feedback.textContent="Reminder prepared for "+name+" · delivery not connected";setTimeout(()=>feedback.textContent="",2500)};
...
function renderRouteWorkspace(){
 ...
 if(!RouteViews[view]){const u=new URL(location.href);u.searchParams.set("view","home");u.searchParams.delete("pool");history.replaceState({route:"home",pool:""},"",u);LinksState.write({route:"home",pool:""});document.documentElement.dataset.view="home";w.innerHTML="";return} w.innerHTML=RouteViews[view]();
...
function openPoolHub(name){
 const p=PoolHubData[name];if(!p){const u=new URL(location.href);u.searchParams.set("view","my-pools");u.searchParams.delete("pool");history.replaceState({route:"my-pools",pool:""},"",u);LinksState.write({route:"my-pools",pool:""});document.documentElement.dataset.view="my-pools";delete document.documentElement.dataset.pool;renderRouteWorkspace();AppStatus.show("warn","Pool not found","LINKS did not substitute a different pool.");return}
 ...
 <section class="poolhub-body">
  <div class="hub-overview" data-hubpanel="overview">
   ...
   <article class="hub-activity"><span>POOL PULSE</span><h3>Pool activity</h3><p><b>No activity yet.</b> <small>Real pool events will appear here.</small></p></article>
  </div>
...
function poolTabPanel(tab,pool){
 ...
 if(tab==="room") return '<div class="hub-panel-head"><span>POOL ROOM</span><h3>Game day conversation.</h3><p>Commissioner updates and pool chatter without exposing protected picks.</p></div><div class="hub-chat"><div class="links-empty"><h3>NO MESSAGES YET</h3><p>Commissioner and player messages will appear here when messaging is connected.</p></div></div>';
...
function wireCommissionerV3(ws){
 ws.querySelector('[data-cmd="remind"]')?.addEventListener("click",()=>{[] .forEach(n=>ReminderQueue.send?.(n));AuditLog.add("REMINDER PREPARED","No seeded recipients");modal("REMINDERS",'<div class="connected-modal"><h3>No missing-pick recipients.</h3><p>Real player status will determine reminder recipients.</p></div>')});
...
function autopilotPanel(){return ''}
...
const RoomStore={
 ...
 add(pool,text){let all={};try{all=JSON.parse(localStorage.getItem(this.key)||"{}")}catch{};const arr=all[pool]||[];arr.push({who:"Player",text:String(text).slice(0,280),at:new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})});all[pool]=arr.slice(-40);localStorage.setItem(this.key,JSON.stringify(all));return all[pool]}
};
...
function patchStandingsTruth(){
 ...
 document.querySelectorAll(".hub-standings").forEach(host=>{
   host.innerHTML=ranked.map((x,i)=>'<div class="'+(x.p.name===currentPlayerNameV172()?"me":"")+'"><strong>'+(i+1)+'</strong><b>'+linksEscape(x.p.name)+'</b><span>'+x.s.wins+' WINS</span><em>'+(x.p.name===currentPlayerNameV172()?"YOU":"")+'</em></div>').join("");
 });
}
...
const JoinGateway={
 ...
 auth(pool,isNew){
  modal(isNew?"JOIN "+pool:"SIGN IN TO LINKS",'<div class="join-auth"><div class="join-auth-mark">L</div><span>'+(isNew?"PLAYER SETUP":"WELCOME BACK")+'</span><h3>'+(isNew?"Join "+linksEscape(pool):"Open "+linksEscape(pool))+'</h3><p>'+(isNew?"Use your email to create your LINKS player profile.":"Sign in and LINKS will return you directly to this invitation.")+'</p><label><span>EMAIL</span><input type="email" data-join-email autocomplete="email" placeholder="you@example.com"></label>'+(isNew?'<label><span>DISPLAY NAME</span><input data-join-name maxlength="28" placeholder="Your name"></label>':'<label><span>PASSWORD</span><input type="password" data-join-pass autocomplete="current-password" placeholder="Password"></label>')+'<button data-join-continue>CONTINUE TO POOL ›</button><small>Account access · authentication connection required before launch.</small></div>');
...
function finalResultsBoard(){
 const ranked=rankedDemoPlayers(),me=ranked.findIndex(x=>x.p.name===currentPlayerNameV172()),leader=ranked[0];
 return '<section class="results-board-v13"><div class="rbv-hero"><div><span>WEEK 3 · RESULTS</span><h2>Scoreboard</h2><p>One clean view of the week, your position and the current leader.</p></div><div class="rbv-leader"><small>LEADER</small><b>'+linksEscape(leader?.p.name||"—")+'</b><span>'+(leader?.s.wins||0)+' WINS</span></div></div><div class="rbv-table"><div class="rbv-row head"><span>RANK</span><span>PLAYER</span><span>CORRECT</span><span>STATUS</span></div>'+ranked.map((x,i)=>'<div class="rbv-row '+(x.p.name===currentPlayerNameV172()?"me":"")+'"><strong>'+(i+1)+'</strong><b>'+linksEscape(x.p.name)+'</b><span>'+x.s.wins+'</span><em>'+(x.p.name===currentPlayerNameV172()?"YOU":i===0?"LEADER":"—")+'</em></div>').join("")+'</div><div class="rbv-you"><div><span>YOUR POSITION</span><b>#'+(me+1)+'</b></div><div><span>YOUR SCORE</span><b>'+(me>=0?ranked[me].s.wins:0)+'</b></div><div><span>GAMES FINAL</span><b>'+ScoreDemo.games.filter(g=>g.status==="final").length+'/'+ScoreDemo.games.length+'</b></div><button data-rbv-picks>REVIEW PICKS ›</button></div></section>';
}
...
function resultMomentum(){
 ...
 const ranked=rankedDemoPlayers(),me=ranked.find(x=>x.p.name===currentPlayerNameV172()),lead=ranked[0],gap=Math.max(0,(lead?.s.wins||0)-(me?.s.wins||0));
...
// Front Door Finish v129 — premium entry surfaces + restrained sponsor placement.
function frontDoorFinishV129(){document.querySelectorAll('.modalbox:not([data-frontdoor-v129])').forEach(m=>{const t=(m.querySelector('h2')?.textContent||'').toLowerCase();if(!/(sign in|join|pool|password|welcome)/.test(t))return;m.dataset.frontdoorV129='1';m.classList.add('frontdoor-v129');m.insertAdjacentHTML('afterbegin','<div class="fd-brand-v129"><b>LINKS</b><span>SECURE POOL ACCESS</span></div>')})}
function sponsorSlotV129(){const view=document.documentElement.dataset.view;if(!['home','results'].includes(view)||document.querySelector('.sponsor-slot-v129'))return;const host=document.querySelector('.main,.route-workspace,.home-command-v4');if(!host)return;const slot=document.createElement('aside');slot.className='sponsor-slot-v129';slot.setAttribute('aria-label','Sponsor');slot.innerHTML='<span>SPONSOR</span><div data-links-ad-slot="launch-footer"><small>Partner message</small></div>';host.appendChild(slot)}
...
// Truth Sweep v132 — remove legacy demo pulse/impact claims from real pool screens.
function poolTruthSweepV132(){if(document.documentElement.dataset.view!=='pool')return;const pool=document.documentElement.dataset.pool||'',p=PoolHubData[pool];if(!p)return;const ready=Number(p.ready)||0,members=Number(p.members)||0,lock=nextLockV114(pool)||'NOT SET',ps=pickStatusV113(pool,p.game);document.querySelectorAll('.hub-live').forEach(x=>{x.classList.add('truth-v132');x.innerHTML='<span>GAME DAY STATUS</span><h3>'+(engineReadyV113(pool)?'Pool is ready.':'Commissioner setup needed.')+'</h3><div><b>'+ready+'</b><small>players submitted</small></div><div><b>'+Math.max(0,members-ready)+'</b><small>still need picks</small></div>'});document.querySelectorAll('.hub-activity').forEach(x=>{x.classList.add('truth-v132');x.innerHTML='<span>POOL STATUS</span><h3>What matters now</h3><p><b>'+(ps.due?ps.due+' PICKS DUE':'YOUR CARD IS COMPLETE')+'</b></p><p><b>NEXT LOCK</b> <small>'+linksEscape(lock)+'</small></p><p><b>LINKS</b> <small>Updates from saved pool data</small></p>'});document.querySelectorAll('.hub-scoreline small').forEach(x=>{if(/live/i.test(x.textContent))x.textContent='Saved pool standing'});document.querySelectorAll('.hub-tile small').forEach(x=>{if(/updates live/i.test(x.textContent))x.textContent='From recorded results'})}
function releaseV132(){poolTruthSweepV132();document.documentElement.dataset.linksBuild='v132';document.querySelectorAll('.app-build-v18').forEach(x=>x.innerHTML='<b>LINKS</b><span>NEW BUILD · v132 · TRUTH SWEEP</span>')}
let truthTickV132;const truthObserverV132=new MutationObserver(()=>{clearTimeout(truthTickV132);truthTickV132=setTimeout(releaseV132,30)});truthObserverV132.observe(document.querySelector('#app'),{childList:true,subtree:true});LinksLifecycleCallbacksV82.push(releaseV132);queueMicrotask(()=>{releaseV132();LinksLifecycleV82.queue()});

// Engine Reconciliation v133 — later real Golf/NASCAR engines always outrank the old v108 waiting screen.
function reconcileRealEnginesV133(){if(document.documentElement.dataset.view!=='pool')return;const pool=document.documentElement.dataset.pool||'',p=PoolHubData[pool];if(!p)return;const type=gameIdentity(p.game||'').type,live=document.querySelector('.pool-tab-live');if(!live)return;const active=[...document.querySelectorAll('.poolhub-tabs button,.pool-tabs button')].find(b=>b.classList.contains('active'))?.textContent.trim().toLowerCase();if(active!=='picks')return;if(type==='golf'&&GolfPoolV109.read(pool).published){if(live.querySelector('.format-honest-v108')||!live.querySelector('.golf-pick-v109')){live.dataset.formatAdapter='1';live.innerHTML=golfPickV109(pool);wireGolfPicksV109(live,pool)}}if(type==='racing'&&NascarPoolV110.read(pool).published){if(live.querySelector('.format-honest-v108')||!live.querySelector('.nascar-pick-v110')){live.dataset.formatAdapter='1';live.innerHTML=nascarPickV110(pool);wireNascarPicksV110(live,pool)}}}
function releaseV133(){reconcileRealEnginesV133();document.documentElement.dataset.linksBuild='v133';document.querySelectorAll('.app-build-v18').forEach(x=>x.innerHTML='<b>LINKS</b><span>NEW BUILD · v133 · ENGINE RECONCILIATION</span>')}
let reconcileTickV133;const reconcileObserverV133=new MutationObserver(()=>{clearTimeout(reconcileTickV133);reconcileTickV133=setTimeout(releaseV133,35)});reconcileObserverV133.observe(document.querySelector('#app'),{childList:true,subtree:true});document.addEventListener('click',e=>{if(e.target.closest('[data-hubtab="picks"]'))setTimeout(reconcileRealEnginesV133,50)},true);LinksLifecycleCallbacksV82.push(releaseV133);queueMicrotask(()=>{releaseV133();LinksLifecycleV82.queue()});

// Playmaker v134 — premium name and entry point for the existing LINKS research card.
function playmakerV134(){document.querySelectorAll('[data-card-builder-v119]').forEach(x=>{x.classList.add('playmaker-v134');x.querySelectorAll('h2,h3,b,span').forEach(n=>{if(/card builder/i.test(n.textContent))n.textContent=n.textContent.replace(/card builder/ig,'PLAYMAKER')})});document.querySelectorAll('.ai-mix-dock-v71 span').forEach(x=>x.textContent='PLAYMAKER CARD');if(document.documentElement.dataset.view==='home'&&!document.querySelector('.playmaker-mark-v134')){const x=document.querySelector('[data-card-builder-v119]');if(x)x.insertAdjacentHTML('afterbegin','<div class="playmaker-mark-v134"><span>LINKS PLAYMAKER</span><b>BUILD THE PLAY. SEE THE WHOLE BOARD.</b><small>Multi-sport research · saved card · review</small></div>')}document.documentElement.dataset.linksBuild='v134';document.querySelectorAll('.app-build-v18').forEach(x=>x.innerHTML='<b>LINKS</b><span>NEW BUILD · v134 · PLAYMAKER</span>')}
let playmakerTickV134;const playmakerObserverV134=new MutationObserver(()=>{clearTimeout(playmakerTickV134);playmakerTickV134=setTimeout(playmakerV134,40)});playmakerObserverV134.observe(document.querySelector('#app'),{childList:true,subtree:true});LinksLifecycleCallbacksV82.push(playmakerV134);queueMicrotask(()=>{playmakerV134();LinksLifecycleV82.queue()});

// Invite Integrity v135 — never claim an email was sent when the New Build has no delivery backend.
function inviteIntegrityV135(){document.querySelectorAll('.invite-direct:not([data-v135])').forEach(f=>{f.dataset.v135='1';const btn=f.querySelector('button');if(btn)btn.textContent='PREPARE INVITE';const input=f.querySelector('input');if(input)input.placeholder='player@email.com or mobile number'});document.querySelectorAll('.invite-v2 .invite-history em').forEach(x=>{if(x.textContent.trim()==='SENT')x.textContent='PREPARED'});document.querySelectorAll('.invite-v2').forEach(h=>{if(!h.querySelector('.invite-note-v135'))h.insertAdjacentHTML('beforeend','<div class="invite-note-v135"><b>INVITE DELIVERY</b><span>Share Link and Email open your device sharing tools. Direct server email delivery will only show as sent after a delivery service is connected.</span></div>')})}
function releaseV135(){inviteIntegrityV135();document.documentElement.dataset.linksBuild='v135';document.querySelectorAll('.app-build-v18').forEach(x=>x.innerHTML='<b>LINKS</b><span>NEW BUILD · v135 · INVITE INTEGRITY</span>')}
let inviteTickV135;const inviteObserverV135=new MutationObserver(()=>{clearTimeout(inviteTickV135);inviteTickV135=setTimeout(releaseV135,35)});inviteObserverV135.observe(document.querySelector('#app'),{childList:true,subtree:true});LinksLifecycleCallbacksV82.push(releaseV135);queueMicrotask(()=>{releaseV135();LinksLifecycleV82.queue()});

// Observer Consolidation v136 — retire recent duplicate observers and use one debounced launch finisher.
function launchFinishV136(){try{cardPulseSyncV130()}catch{}try{modalFinishV131()}catch{}try{poolTruthSweepV132()}catch{}try{reconcileRealEnginesV133()}catch{}try{playmakerV134()}catch{}try{inviteIntegrityV135()}catch{}document.documentElement.dataset.linksBuild='v136';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v136 · LAUNCH INTEGRITY</span>';if(x.innerHTML!==h)x.innerHTML=h})}
['routeObserverV124','releaseObserverV126','qaObserverV127','revealObserverV128','frontDoorObserverV129','gateObserverV130','releaseObserverV131','truthObserverV132','reconcileObserverV133','playmakerObserverV134','inviteObserverV135'].forEach(n=>{try{eval(n)?.disconnect()}catch{}})
...
// Truth Guard v138 — legacy demo standings/pool patches must never overwrite real created-pool data.
try{standingTruthObserver.disconnect()}catch{}try{poolTruthObserver.disconnect()}catch{}try{picksTruthObserver.disconnect()}catch{}try{resultObserver.disconnect()}catch{}
function realStandingsGuardV138(){if(document.documentElement.dataset.view!=='pool')return;const pool=document.documentElement.dataset.pool||'';if(!CreatedPools.all().some(x=>x.name===pool))return;const type=gameIdentity(PoolHubData[pool]?.game||'').type;if(['golf','racing'].includes(type)){try{eventStandingsV116(pool)}catch{}}}
function truthGuardV138(){realStandingsGuardV138();document.documentElement.dataset.linksBuild='v138';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v138 · REAL DATA GUARD</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(truthGuardV138);queueMicrotask(()=>{truthGuardV138();LinksLifecycleV82.queue()});

// Fantasy/Dynasty Launch Guard v139 — do not market unfinished engines as live playable products.
function fantasyLaunchGuardV139(){document.querySelectorAll('[data-network-game]').forEach(b=>{const n=(b.dataset.networkGame||'').toUpperCase();if(!/DYNASTY|FANTASY/.test(n))return;b.dataset.v139='1';const d=b.querySelector('small,span');if(d&&/draft|contract|waiver|trade|playoff/i.test(d.textContent))d.textContent='FULL ENGINE · FINAL QA'})}
document.addEventListener('click',e=>{const b=e.target.closest('[data-network-game]');if(!b)return;const n=(b.dataset.networkGame||'').toUpperCase();if(!/DYNASTY|FANTASY/.test(n))return;e.preventDefault();e.stopImmediatePropagation();modal('FINAL ENGINE QA','<div class="connected-modal"><span class="badge">FANTASY FOOTBALL</span><h3>'+linksEscape(b.dataset.networkGame)+'</h3><p>This full-season engine is still in final launch verification. LINKS will not open a demo roster or pretend league management is live before draft, roster, scoring, waivers, trades and standings are connected end to end.</p></div>')},true);
function releaseV139(){fantasyLaunchGuardV139();document.documentElement.dataset.linksBuild='v139';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v139 · FANTASY QA GATE</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV139);queueMicrotask(()=>{releaseV139();LinksLifecycleV82.queue()});

// Playmaker Truth Gate v140 — remove illustrative market samples from the launch-facing research experience.
function playmakerTruthV140(){document.querySelectorAll('.ai-ticket,.ai-research-board,.ai-studio-v19').forEach(host=>{host.querySelectorAll('[data-leg],.ticketlegs>div').forEach(row=>{const t=row.textContent||'';if(/PLAYER A|PLAYER B|FIGHTER A|TOUR MATCH|Preview research sample|Illustrative/i.test(t))row.remove()})});document.querySelectorAll('.playmaker-mark-v134 small').forEach(x=>x.textContent='Multi-sport research · saved card · review · verified data only')}
const applyAISportV66Preview=applyAISportV66;applyAISportV66=function(sport){aiSportV66=sport;AIStudio.legs=[];AIStudio.save([]);const h=document.querySelector('#aiStudioMount');if(!h)return;h.innerHTML=AIStudio.html();AIStudio.wire(h);h.insertAdjacentHTML('afterbegin',aiPlanStrip());h.querySelector('.ai-studio-v19')?.insertAdjacentHTML('afterbegin',multiSportBar());const board=h.querySelector('.ai-board,.ai-research-board,.ai-studio-v19');if(board&&!board.querySelector('.playmaker-feed-gate-v140'))board.insertAdjacentHTML('afterbegin','<div class="playmaker-feed-gate-v140"><b>LIVE DATA REQUIRED</b><span>'+linksEscape(sport)+' research will populate only from a connected current market/sports-data source. LINKS will not substitute sample odds or invented players.</span></div>');h.querySelectorAll('[data-ai-sport]').forEach(b=>b.classList.toggle('active',b.dataset.aiSport===sport));playmakerTruthV140()}
function releaseV140(){playmakerTruthV140();document.documentElement.dataset.linksBuild='v140';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v140 · PLAYMAKER TRUTH</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV140);queueMicrotask(()=>{releaseV140();LinksLifecycleV82.queue()});

// Sportsbook Match v141 — book-first Playmaker architecture; affiliates/marks stay disabled until approved.
const PlaymakerBooksV141={key:'links-playmaker-book-v141',read(){return localStorage.getItem(this.key)||''},write(v){localStorage.setItem(this.key,v);return v},rows:{draftkings:{name:'DraftKings'},fanduel:{name:'FanDuel'},bet365:{name:'bet365'}}};
Object.keys(LinksAffiliateV70.partners).forEach(id=>{LinksAffiliateV70.partners[id].enabled=false;LinksAffiliateV70.partners[id].affiliate=''});
function playmakerBookChooserV141(){const h=document.querySelector('#aiStudioMount');if(!h||h.querySelector('.pm-book-v141'))return;const selected=PlaymakerBooksV141.read();h.insertAdjacentHTML('afterbegin','<section class="pm-book-v141"><span>01 · CHOOSE YOUR SPORTSBOOK</span><h3>Build against the book you actually use.</h3><p>Odds differ by sportsbook. LINKS will only show a book’s lines when an approved live data source is connected for that book.</p><div>'+Object.entries(PlaymakerBooksV141.rows).map(([id,x])=>'<button data-pm-book="'+id+'" class="'+(selected===id?'active':'')+'"><i>'+linksEscape(x.name.slice(0,3).toUpperCase())+'</i><b>'+linksEscape(x.name)+'</b><small>'+(selected===id?'SELECTED · DATA CONNECTION PENDING':'SELECT BOOK')+'</small></button>').join('')+'</div><em>Sportsbook logos and affiliate tracking remain off until LINKS has the applicable partner approval and approved creative/link assets.</em></section>')}
document.addEventListener('click',e=>{const b=e.target.closest('[data-pm-book]');if(!b)return;e.preventDefault();const id=PlaymakerBooksV141.write(b.dataset.pmBook);document.querySelectorAll('[data-pm-book]').forEach(x=>{x.classList.toggle('active',x.dataset.pmBook===id);const s=x.querySelector('small');if(s)s.textContent=x.dataset.pmBook===id?'SELECTED · DATA CONNECTION PENDING':'SELECT BOOK'});AppStatus.show('ok',PlaymakerBooksV141.rows[id].name+' selected','Playmaker will use this book only when its approved current odds feed is connected.')},true);
function affiliateGuardV141(){document.querySelectorAll('.book-handoff-v22 [data-book]').forEach(b=>{const id=b.dataset.book,p=LinksAffiliateV70.partners[id];b.disabled=!(p&&p.enabled&&p.affiliate);const s=b.querySelector('span');if(s&&!p?.affiliate)s.textContent='PARTNER SETUP PENDING'})}
function releaseV141(){playmakerBookChooserV141();affiliateGuardV141();document.documentElement.dataset.linksBuild='v141';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v141 · SPORTSBOOK MATCH</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV141);queueMicrotask(()=>{releaseV141();LinksLifecycleV82.queue()});

// Real Pool Isolation v142 — no created pool may inherit DemoSlate/GameAdapters sample matchups.
const normalizedSlateV86Legacy=normalizedSlateV86;
normalizedSlateV86=function(pool){const configured=GameSlateStoreV86.read(pool);if(configured.length)return configured;const created=CreatedPools.all().some(x=>x.name===pool);if(created)return[];return normalizedSlateV86Legacy(pool)};
poolSlate=function(pool){return normalizedSlateV86(pool)};
function demoIsolationV142(){if(document.documentElement.dataset.view!=='pool')return;const pool=document.documentElement.dataset.pool||'';if(!CreatedPools.all().some(x=>x.name===pool))return;document.querySelectorAll('.slate-engine,.score-engine,.results-truth').forEach(x=>x.remove());const type=gameIdentity(PoolHubData[pool]?.game||'').type;if(['football','college','confidence','survivor','game33'].includes(type)&&!GameSlateStoreV86.has(pool)){const live=document.querySelector('.pool-tab-live');if(live&&!live.querySelector('.links-empty')){live.innerHTML='<div class="hub-panel-head"><span>REAL GAME DATA</span><h3>Waiting for this pool’s slate.</h3><p>LINKS will not substitute demonstration teams, kickoff times or results in a real pool.</p></div><div class="links-empty picks"><i>⚙</i><h3>SLATE NOT PUBLISHED</h3><p>Commissioner setup must publish the actual games before selections open.</p></div>'}}}
function releaseV142(){demoIsolationV142();document.documentElement.dataset.linksBuild='v142';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v142 · REAL POOL ISOLATION</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV142);queueMicrotask(()=>{releaseV142();LinksLifecycleV82.queue()});

// Lock Enforcement v143 — locked games cannot save through any adapter/event path.
const PickEngineSaveV143=PickEngine.save.bind(PickEngine);
PickEngine.save=function(game,team){const pool=document.documentElement.dataset.pool||new URL(location.href).searchParams.get('pool')||'';const games=GameSlateStoreV86.read(pool);const raw=String(game||'');const g=games.find(x=>raw===pool+'-'+x.id||raw==='slate-'+x.id||raw===x.id);if(g&&gameLockedV94(g)){AppStatus.show('warn','Pick locked',(g.away||'')+' vs '+(g.home||'')+' has already kicked off. Your saved selection is protected.');return null}return PickEngineSaveV143(game,team)};
function lockInteractionV143(){if(document.documentElement.dataset.view!=='pool')return;const pool=document.documentElement.dataset.pool||'',games=GameSlateStoreV86.read(pool);if(!games.length)return;for(const g of games){if(!gameLockedV94(g))continue;document.querySelectorAll('[data-adapter-game="'+CSS.escape(g.id)+'"],[data-confidence-game="'+CSS.escape(g.id)+'"],[data-slate="'+CSS.escape(g.id)+'"]').forEach(b=>{b.disabled=true;b.classList.add('kickoff-locked-v94');b.setAttribute('aria-disabled','true');b.setAttribute('title','Locked at kickoff')})}}
document.addEventListener('click',e=>{const b=e.target.closest('[data-adapter-game],[data-confidence-game],[data-slate]');if(!b)return;const pool=document.documentElement.dataset.pool||'',id=b.dataset.adapterGame||b.dataset.confidenceGame||b.dataset.slate,g=GameSlateStoreV86.read(pool).find(x=>x.id===id);if(g&&gameLockedV94(g)){e.preventDefault();e.stopImmediatePropagation();AppStatus.show('warn','Game locked','This selection became read-only at kickoff.')}},true);
function releaseV143(){lockInteractionV143();document.documentElement.dataset.linksBuild='v143';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v143 · HARD KICKOFF LOCKS</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV143);queueMicrotask(()=>{releaseV143();LinksLifecycleV82.queue()});

// Pool Persistence Integrity v144 — remove the hidden 20-pool truncation and make local prototype writes duplicate-safe.
CreatedPools.save=function(p){const a=this.all().filter(x=>x.id!==p.id&&String(x.name||'').toLowerCase()!==String(p.name||'').toLowerCase());a.unshift(p);try{localStorage.setItem(this.key,JSON.stringify(a))}catch(err){AppStatus.show('warn','Pool storage full','This test build could not save another local pool. No existing pool was deleted.');throw err}return p};
function poolPersistenceAuditV144(){const rows=CreatedPools.all(),seen=new Set(),clean=[];for(const p of rows){const k=String(p.name||'').trim().toLowerCase();if(!k||seen.has(k))continue;seen.add(k);clean.push(p)}if(clean.length!==rows.length)try{localStorage.setItem(CreatedPools.key,JSON.stringify(clean))}catch{}}
function releaseV144(){poolPersistenceAuditV144();document.documentElement.dataset.linksBuild='v144';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v144 · POOL PERSISTENCE</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV144);queueMicrotask(()=>{releaseV144();LinksLifecycleV82.queue()});

// Launch Catalog Integrity v145 — only formats with a verified New Build engine can be created.
const LaunchFormatsV145=new Set(['football','college','survivor','confidence','squares','bracket','golf','racing','game33']);
function launchFormatTypeV145(name){return gameIdentity(name||'').type}
function catalogIntegrityV145(){document.querySelectorAll('[data-network-game],[data-create-game]').forEach(b=>{const type=launchFormatTypeV145(b.dataset.networkGame||b.dataset.createGame);const ready=LaunchFormatsV145.has(type);b.classList.toggle('launch-hold-v145',!ready);if(!ready){b.setAttribute('aria-label',(b.dataset.networkGame||b.dataset.createGame)+' — in development');const copy=b.querySelector('.network-copy b,.network-copy span:last-child,em');if(copy&&/OPEN GAME|›/.test(copy.textContent))copy.textContent='IN DEVELOPMENT'}})}
document.addEventListener('click',e=>{const b=e.target.closest('[data-network-game],[data-create-game]');if(!b)return;const name=b.dataset.networkGame||b.dataset.createGame,type=launchFormatTypeV145(name);if(LaunchFormatsV145.has(type))return;e.preventDefault();e.stopImmediatePropagation();modal('ENGINE IN FINAL DEVELOPMENT','<div class="connected-modal"><span class="badge">LINKS GAME NETWORK</span><h3>'+linksEscape(name)+'</h3><p>This format is visible in the LINKS network, but pool creation stays closed until its complete game engine passes launch QA. No placeholder pool will be created.</p></div>')},true);
...
// Package Truth v148 — generous free-pool launch model; no paid pool gate until billing/package policy is finalized.
const LinksPoolPlanV148={freeActivePools:3,label:'FREE FOR LIFE'};
function packageTruthV148(){document.querySelectorAll('.pricing-card').forEach(card=>{card.innerHTML='<div class="section-title"><h2>LINKS POOLS</h2><span class="badge live">FREE FOR LIFE</span></div><div class="plans"><div class="featured"><b>FREE FOR LIFE</b><strong>Run up to '+LinksPoolPlanV148.freeActivePools+' active pools</strong><span>Core games · commissioner tools · player access</span></div><div><b>MORE POOLS</b><strong>Future commissioner upgrade</strong><span>For commissioners who outgrow the free allowance · pricing not set</span></div><div><b>PLAYMAKER PRO</b><strong>Future research upgrade</strong><span>Expanded research tools · billing not live</span></div></div><p class="muted plan-note">Players never pay just to join a pool or submit picks. No paid pool package is being enforced in this New Build.</p>'})}
const CreatePoolOpenV148=CreatePoolStudio.open.bind(CreatePoolStudio);CreatePoolStudio.open=function(game){CreatePoolOpenV148(game);queueMicrotask(()=>{const n=CreatedPools.all().length;if(n>=LinksPoolPlanV148.freeActivePools){const h=document.querySelector('#createPoolMount');if(h)h.insertAdjacentHTML('afterbegin','<div class="package-note-v148"><b>TEST BUILD · PACKAGE LIMIT NOT ENFORCED</b><span>You have '+n+' locally created pool'+(n===1?'':'s')+'. LINKS will not block creation until accounts, billing and active-pool rules are connected to the backend.</span></div>')}})};
function releaseV148(){packageTruthV148();document.documentElement.dataset.linksBuild='v148';document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v148 · FREE FOR LIFE</span>';if(x.innerHTML!==h)x.innerHTML=h})}
LinksLifecycleCallbacksV82.push(releaseV148);queueMicrotask(()=>{releaseV148();LinksLifecycleV82.queue()});

// Reveal QA v151 — truth-first launch hardening.
function revealQaV151(){
 document.querySelectorAll('.home-live-strip,.home-brief').forEach(x=>x.remove());
 document.querySelectorAll('[data-remind],[data-remind-all]').forEach(b=>{b.title='Prepares a local reminder draft. No email or text is sent until delivery is connected.'});
 document.querySelectorAll('[data-links-owner],[data-global-admin]').forEach(b=>{b.setAttribute('aria-disabled','true');b.title='LINKS Owner authentication requires the production backend.'});
 document.documentElement.dataset.linksBuild='v151';
...
// Deep Clean v171 — seed registries, prototype pool room messages and personal-name defaults removed.
function sourceTruthV171(){
 const created=CreatedPools.all();
 if(!created.length){try{localStorage.removeItem('links-room-v2')}catch{}}
 document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v171 · SOURCE CLEAN</span>';if(x.innerHTML!==h)x.innerHTML=h});
 document.documentElement.dataset.linksBuild='v171';
}
LinksLifecycleCallbacksV82.push(sourceTruthV171);
queueMicrotask(()=>{sourceTruthV171();LinksLifecycleV82.queue()});


// Identity Truth v172 — no hard-coded personal identity in presentation logic.
function currentPlayerNameV172(){
 try{return localStorage.getItem('links-player-name')||localStorage.getItem('links-display-name')||'Player'}catch{return 'Player'}
}
function identityTruthV172(){
 document.querySelectorAll('.hub-chat,.room-feed,.rivalry-card').forEach(x=>{
   if(/Player 2|Player 3|Jake vs Mike|Week 3 is open/i.test(x.textContent||''))x.remove();
 });
 document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v172 · IDENTITY TRUTH</span>';if(x.innerHTML!==h)x.innerHTML=h});
 document.documentElement.dataset.linksBuild='v172';
}
LinksLifecycleCallbacksV82.push(identityTruthV172);
queueMicrotask(()=>{identityTruthV172();LinksLifecycleV82.queue()});


// Seed Purge v173 — old activity, chat and reminder recipients removed from source.
function seedPurgeV173(){
 document.querySelectorAll('[data-preview="true"]').forEach(x=>x.removeAttribute('data-preview'));
 document.querySelectorAll('.v42-preview').forEach(x=>x.remove());
 document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v173 · SEED PURGE</span>';if(x.innerHTML!==h)x.innerHTML=h});
 document.documentElement.dataset.linksBuild='v173';
}
...
// Reveal Polish v177 — fake autopilot and signed-in command prototypes retired; public mockup made neutral.
function revealPolishV177(){
 document.querySelectorAll('.autopilot-v1').forEach(x=>x.remove());
 document.querySelectorAll('.public-phone').forEach(x=>{if(/JAKE|1 PICK LEFT|My Pool/i.test(x.textContent||''))x.remove()});
 document.querySelectorAll('.app-build-v18').forEach(x=>{const h='<b>LINKS</b><span>NEW BUILD · v177 · REVEAL POLISH</span>';if(x.innerHTML!==h)x.innerHTML=h});
 document.documentElement.dataset.linksBuild='v177';
}

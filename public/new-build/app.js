...
const PoolHubData={
 "Sunday Crew":{icon:"🏈",game:"NFL PICK’EM",week:"WEEK 3",members:64,ready:59,lock:"THU · 7:15 PM",record:"2–0",rank:"T-1",accent:"NFL"},
...
function openPoolHub(name){
 const p=PoolHubData[name]||PoolHubData["Sunday Crew"];
...
function mountNowStrip(){
 ...
 n.innerHTML='<span class="now-pulse"></span><b>NOW</b><div class="now-copy"><strong>1 PICK DUE</strong><span>Sunday Crew · Thu 7:15 PM</span></div><button data-now-action>FINISH ›</button>';
...
function mountPoolGameDay(){
 ...
 const pool=document.documentElement.dataset.pool||new URL(location.href).searchParams.get("pool")||"Sunday Crew";
 const p=PoolHubData[pool]||PoolHubData["Sunday Crew"];
...
const InboxStore={
 key:"links-inbox-v2",
 seed:[
  {id:"lock",type:"urgent",icon:"⏱",title:"1 pick still missing",body:"Sunday Crew · TEN at IND locks Thursday at 7:15 PM.",time:"NOW",action:"PICKS",read:false},
  ...
  {id:"result",type:"result",icon:"★",title:"You moved into T-1",body:"Two finals moved you up one spot in Sunday Crew.",time:"42m",action:"RESULTS",read:true},
...
const MyPickCards=[
 {pool:"Sunday Crew",game:"NFL PICK’EM",due:"THU · 7:15 PM",done:2,total:3,state:"action",note:"1 PICK MISSING"},
...
document.addEventListener("click",e=>{
 ...
 const pool=hub?.textContent?.trim()||new URL(location.href).searchParams.get("pool")||"Sunday Crew";
...
function inviteCenter(pool="Sunday Crew"){
...
document.addEventListener("click",e=>{
 ...
 if(!b)return;const pool=new URL(location.href).searchParams.get("pool")||"Sunday Crew";e.preventDefault();e.stopImmediatePropagation();openInviteCenter(pool);
},true);
...
document.addEventListener("submit",e=>{
 ...
 const pool=new URL(location.href).searchParams.get("pool")||document.documentElement.dataset.pool||"Sunday Crew";
...
function homeBrief(){
 const unread=InboxStore.all().filter(x=>!x.read).length;
 return '<section class="home-brief"><div class="hb-head"><div><span>YOUR LINKS BRIEF</span><h2>What matters right now.</h2></div><button data-brief-inbox>'+unread+' UNREAD ›</button></div><div class="hb-grid"><button data-home-go2="my-picks"><i class="pick">✓</i><div><span>PICKS</span><b>Finish Sunday Crew</b><small>1 selection left before the next lock</small></div><em>DO IT ›</em></button><button data-home-go2="notifications"><i class="msg">✦</i><div><span>INBOX</span><b>Commissioner update</b><small>Week 3 access and deadline information</small></div><em>READ ›</em></button><button data-home-go2="results"><i class="move">↑</i><div><span>MOVEMENT</span><b>You moved into the top group</b><small>Live standings changed after the last final</small></div><em>VIEW ›</em></button></div></section>';
}
...
function patchMyPoolsTruth(){
 ...
 host.querySelectorAll(".portfolio-card").forEach(card=>{
   ...
   if(name==="Sunday Crew")em.textContent=s.due?(s.due+" PICK"+(s.due===1?"":"S")+" DUE ›"):"OPEN POOL ›";else em.textContent="OPEN POOL ›";
   card.classList.toggle("attention",name==="Sunday Crew"&&s.due>0);
 });
}
...
function patchMyPicksTruth(){
 ...
 const barnes=host.querySelector('[data-pickpool="Sunday Crew"]');
 if(barnes){
...
function mountPickReview(){
 ...
 w.querySelectorAll("[data-prv-game],[data-prv-review]").forEach(b=>b.onclick=()=>LinksRouter.navigate("pool","Sunday Crew"));
}
...
function gameCenterV21(pool){
 const p=PoolHubData[pool]||PoolHubData["Sunday Crew"],g=DemoSlate[0],saved=PickEngine.get(g.id)?.team||"",locked=LockEngine.isLocked(g.id);

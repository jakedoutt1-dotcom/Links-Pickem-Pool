const CACHE="links-new-build-shell-v525";
const SHELL=["./","./index.html","./app.css","./app.js","./build.js","./shell.css","./shell.js","./manifest.webmanifest"];
self.addEventListener("install",event=>{self.skipWaiting();event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(SHELL.map(x=>x+"?v=525")).catch(()=>{}))});

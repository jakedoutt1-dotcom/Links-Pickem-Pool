...
async function loadLive(){
 ...
 const settled=await Promise.allSettled(urls.map(async([sport,url])=>{const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),7000);try{const r=await fetch(url,{cache:'no-store',signal:ctl.signal});if(!r.ok)throw Error('feed');const j=await r.json();return (j.events||[]).map(e=>{const comp=e.competitions?.[0],teams=comp?.competitors||[],away=teams.find(x=>x.homeAway==='away'),home=teams.find(x=>x.homeAway==='home');return away&&home?{sport,a:away.team.abbreviation,h:home.team.abbreviation,as:away.score,hs:home.score,state:e.status?.type?.state,detail:e.status?.type?.shortDetail||'',date:new Date(e.date).getTime()}:null}).filter(Boolean)}finally{clearTimeout(timer)}}));
 ...
 let items=games.slice(0,40).map(g=>'<span>'+g.sport+' · '+g.a+' '+(g.state==='pre'?'':g.as)+' <b>'+g.h+' '+(g.state==='pre'?'':g.hs)+'</b><em>'+(g.state==='in'?'● LIVE · ':'')+g.detail+'</em></span>');
 if(!items.length){if(cached)return;track.innerHTML='<span>'+(focus?.label||'LINKS LIVE')+' · Score feed is reconnecting…</span>';track.dataset.loaded='1';return}
 const prefix=focus?'<span><b>'+focus.label+'</b><em>CURRENT GAME WEEK</em></span>':'',html=prefix+items.join('')+prefix+items.join('');
 track.innerHTML=html;track.dataset.loaded='1';try{localStorage.setItem(cacheKey,html)}catch{}
}
loadLive();setInterval(loadLive,180000);document.querySelectorAll('.tabs button').forEach(b=>b.onclick=()=>{b.parentElement.querySelectorAll('button').forEach(x=>x.classList.remove('active'));b.classList.add('active')});
